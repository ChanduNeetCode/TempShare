...
const PeriodDay = props => {
  ...
  const markingStyle = useMemo(() => {
    const defaultStyle = {
      textStyle: style.current.textStyles,
      containerStyle: {},
    };
    if (!marking) {
      return defaultStyle;
      ...
      if (marking.startingDay) {
        // defaultStyle.containerStyle = {borderWidth:1,backgroundColor:'#f0792e'};
        defaultStyle.startingDay = {
          backgroundColor: marking.color
            ? marking.color
            : style.current.middleDays.color,
          borderWidth: 1,
        };
      }
  ...
  const containerStyle = useMemo(() => {
    ...
    if (marking) {
      ...
      if (start && !end) {
        containerStyle.push({
          backgroundColor: style.current.firstAndLastDay.color,
        });
